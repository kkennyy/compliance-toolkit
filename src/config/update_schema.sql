...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(asset_data->>'status', 'Pending'),
    (NULLIF(asset_data->>'investment_amount', '')::decimal),
    (NULLIF(asset_data->>'investment_date', '')::date),
    (NULLIF(asset_data->>'target_exit_date', '')::date)
  )
  ...
  -- Return the created asset with access info
  RETURN (
